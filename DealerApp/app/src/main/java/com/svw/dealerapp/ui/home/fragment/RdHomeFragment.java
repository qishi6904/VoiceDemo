...
import com.svw.dealerapp.ui.home.adapter.RdHomeAdapter;
...
import com.svw.dealerapp.ui.home.presenter.RdHomePresenter;
...
import com.svw.dealerapp.ui.widget.RdHomeHeaderLayout;
import com.svw.dealerapp.ui.widget.RoundCornerLinearLayout;
...
public class RdHomeFragment extends BaseListFragment<HomeEntity, HomeInfoEntity> implements HomeContract.View, View.OnClickListener {

    private static final int updateScheduleRequestCode = 1001;

    //    private HomeAdapter adapter;
    private RdHomeAdapter adapter2;
...
//    private TextView tvTopTipNum;
    ...
    private RdHomeHeaderLayout mTrafficHeaderLayout, mECommerceHeaderLayout, mFollowupHeaderLayout;
    private RelativeLayout mDueRecordsRl, mToBeApprovedRl;
    private TextView mDueRecordsTv, mToBeApprovedTv;
    private ImageView mDueRecordsDismissIv;
    private AutoPlayLayout mAutoPlayView;

    @Override
    public void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        presenter = new RdHomePresenter(this, new HomeModel());
    }

    @Override
    public BaseRecyclerViewAdapter getAdapter() {
//        adapter = new HomeAdapter(getActivity(), presenter.getDataList());
        adapter2 = new RdHomeAdapter(getActivity(), presenter.getDataList());

//        initHeader();
        initHeader2();

        rvRecyclerView.setBackgroundColor(getResources().getColor(R.color.page_bg));
        adapter2.setCallback(new RdHomeAdapter.Callback() {
            @Override
            public void onUpdateClick(HomeInfoEntity entity, int position) {
            ...
            @Override
            public void onCancelClick(HomeInfoEntity entity, int position) {
...
//        if (UserInfoUtils.isManagerPermission()) {
        if(PrivilegeDBUtils.isCheck(Constants.P_SA_APP_HOME, Constants.E_HOME_APPROVAL_SUBVIEW)) {
//            hhlApprove.setVisibility(View.VISIBLE);
            llAutoPlayOuter.setVisibility(View.VISIBLE);
            mAutoPlayView.setVisibility(View.VISIBLE);
            mToBeApprovedRl.setVisibility(View.VISIBLE);
            ((RdHomePresenter) presenter).getAutoPlayData(getActivity(), null);
        } else {
//            hhlApprove.setVisibility(View.GONE);
            llAutoPlayOuter.setVisibility(View.GONE);
            mAutoPlayView.setVisibility(View.GONE);
            mToBeApprovedRl.setVisibility(View.GONE);
        }
...
//    }
    ...
    private void initHeader2() {
        headerView2 = View.inflate(getContext(), R.layout.rd_ui_home_header, null);
        mTrafficHeaderLayout = (RdHomeHeaderLayout) headerView2.findViewById(R.id.rd_ui_home_header_traffic);
        mECommerceHeaderLayout = (RdHomeHeaderLayout) headerView2.findViewById(R.id.rd_ui_home_header_ecommerce);
        mFollowupHeaderLayout = (RdHomeHeaderLayout) headerView2.findViewById(R.id.rd_ui_home_header_followup);
        mTrafficHeaderLayout.setBackgroundResource(R.drawable.rd_home_header_traffic_bg);
        mECommerceHeaderLayout.setBackgroundResource(R.drawable.rd_home_header_commerce_bg);
        mFollowupHeaderLayout.setBackgroundResource(R.drawable.rd_home_header_followup_bg);
        mTrafficHeaderLayout.init(R.mipmap.rd_home_traffic, getString(R.string.rd_home_header_traffic));
        mECommerceHeaderLayout.init(R.mipmap.rd_home_ec, getString(R.string.rd_home_header_ec));
        mFollowupHeaderLayout.init(R.mipmap.rd_home_followup, getString(R.string.rd_home_header_followup));

        mToBeApprovedRl = (RelativeLayout) headerView2.findViewById(R.id.rd_ui_home_to_be_approved_rl);
        mToBeApprovedTv = (TextView) headerView2.findViewById(R.id.rd_ui_home_to_be_approved_tv);
        mDueRecordsRl = (RelativeLayout) headerView2.findViewById(R.id.rd_ui_home_header_due_records_rl);
        mDueRecordsTv = (TextView) headerView2.findViewById(R.id.rd_ui_home_header_due_records_tv);
        mDueRecordsDismissIv = (ImageView) headerView2.findViewById(R.id.rd_ui_home_header_due_records_dismiss_iv);
        mAutoPlayView = (AutoPlayLayout) headerView2.findViewById(R.id.rd_ui_home_auto_play_view);
        tvAutoPlayError = (TextView) headerView2.findViewById(R.id.tv_auto_play_error);
        llAutoPlayError = (LinearLayout) headerView2.findViewById(R.id.ll_auto_play_error_outer);
        llAutoPlayOuter = (RoundCornerLinearLayout) headerView2.findViewById(R.id.rd_ui_home_auto_play_container);
        llNoDataLayout = (LinearLayout) headerView2.findViewById(R.id.ll_error_layout);
        rvRecyclerView.addHeaderView(headerView2);

        mTrafficHeaderLayout.setOnClickListener(this);
        mECommerceHeaderLayout.setOnClickListener(this);
        mFollowupHeaderLayout.setOnClickListener(this);
        mDueRecordsDismissIv.setOnClickListener(this);
        mToBeApprovedRl.setOnClickListener(this);
    }
...
//        rlTopTip.setVisibility(View.GONE);
        mDueRecordsRl.setVisibility(View.GONE);
    }
...
//        tvTopTipNum.setText(topTip);
        mDueRecordsRl.setVisibility(View.VISIBLE);
        mDueRecordsTv.setText(topTip);
    }
    ...
    private void showCancelDialog(final HomeEntity.HomeInfoEntity entity, final int position) {
        if (null == cancelDialog) {
            ...
            dialogAdapter.setContent(getResources().getString(R.string.rd_mine_schedule_cancel_tip));
        }
        cancelDialog.setBtnConfirmText(getString(R.string.public_yes));
        cancelDialog.setBtnCancelText(getString(R.string.public_no));
        cancelDialog.setOnBtnClickListener(new CustomDialog.OnBtnClickListener() {
            ...
            public void onConfirmBtnClick() {
                RdHomePresenter homePresenter = (RdHomePresenter) presenter;
...
//        hhlApprove.setTvHeaderNumber(unapprovedCount);
        mTrafficHeaderLayout.setBigNumber(trafficCount);
        mECommerceHeaderLayout.setBigNumber(eCommerceCount);
        mFollowupHeaderLayout.setBigNumber(followUpCount);
        mToBeApprovedTv.setText(unapprovedCount);
    }
...
//            hhlTraffic.setCircleTip(trafficDelayNum);
            mTrafficHeaderLayout.setTipNumber(trafficDelayNum);
            mTrafficHeaderLayout.showTipNumber(true);
            try {
...
//            hhlTraffic.hideCircleTip();
            mTrafficHeaderLayout.showTipNumber(false);
        }

        if (!TextUtils.isEmpty(eCommerceDelayNum) && !"0".equals(eCommerceDelayNum)) {
//            hhlECommerce.setCircleTip(eCommerceDelayNum);
            mECommerceHeaderLayout.setTipNumber(eCommerceDelayNum);
            mECommerceHeaderLayout.showTipNumber(true);
            try {
...
//            hhlECommerce.hideCircleTip();
            mECommerceHeaderLayout.showTipNumber(false);
        }

        if (!TextUtils.isEmpty(followDelayNum) && !"0".equals(followDelayNum)) {
//            hhlFollowUp.setCircleTip(followDelayNum);
            mFollowupHeaderLayout.setTipNumber(followDelayNum);
            mFollowupHeaderLayout.showTipNumber(true);
            try {
...
//            hhlFollowUp.hideCircleTip();
            mFollowupHeaderLayout.showTipNumber(false);
        }
        ...
        if (totalDelayNum > 0) {
            ...
            mDueRecordsTv.setText(getString(R.string.home_delay_tip_one) +
                    String.valueOf(totalDelayNum) + getString(R.string.home_delay_tip_two));
            mDueRecordsRl.setVisibility(View.VISIBLE);
        } else {
//            rlTopTip.setVisibility(View.GONE);
            mDueRecordsRl.setVisibility(View.GONE);
...
//        isFirstStart = false;
    ...
    public void stopAutoPlay() {
        if (null != mAutoPlayView) {
            mAutoPlayView.stopAutoPlay();
        }
    }

    public void startAutoPlay() {
        if (null != mAutoPlayView) {
            mAutoPlayView.startAutoPlay();
        }
    }
