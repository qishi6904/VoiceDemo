...
/**
 * Created by xupan on 2018/1/16.
 */

public class RdHomePresenter extends ListFragmentPresenter<HomeEntity, HomeEntity.HomeInfoEntity>
    ...
    public RdHomePresenter(ListFragmentContract.View<HomeEntity, HomeInfoEntity> view, ListFragmentContract.Model<ResEntity<HomeEntity>> model) {
        super(view, model);
     ...
     * 剔除已取消的预约
     */
    private void filterCanceledAppointments() {
        if (dataList == null || dataList.isEmpty()) {
            return;
        }
        List<HomeInfoEntity> resultList = new ArrayList<>();//存放未取消的预约list
        int canceledCount = 0;
        for (HomeInfoEntity entity : dataList) {
            if ("17540".equals(entity.getAppmStatusId())) { // 已取消的预约
                canceledCount++;
            } else {
